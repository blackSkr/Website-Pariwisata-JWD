...
// Mengatur zona waktu ke Indonesia (Samarinda)
...
// Mengambil data tambahan dari form
...
$waktuPesanan = date('Y-m-d H:i:s'); // Format: YYYY-MM-DD HH:MM:SS
...
$sql = "UPDATE pelanggan 
        SET jenis_paket = ?, jumlah_pelanggan = ?, tgl_cekin = ?, tgl_cekout = ?, jumlah_hari = ?, total_harga = ?, tambahan = ?, status = ?, waktu_pemesanan = ?
        WHERE id_pelanggan = ?";
...
$stmt->bind_param("sississssi", $jenisPaket, $jumlahPeserta, $checkIn, $checkOut, $jumlahHari, $totalHarga, $tambahan, $status, $waktuPesanan, $id);

// Mengeksekusi statement dan memeriksa apakah berhasil
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        // Redirect jika berhasil
        header("Location: ../user/pesanan-user.php");
